...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var viewMeetingBG: UIView!
    fileprivate var jitsiMeetView: JitsiMeetView?
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        viewMeetingBG.layer.cornerRadius = 5
        viewMeetingBG.layer.masksToBounds = true
        
        videoButton?.layer.cornerRadius = 2
        videoButton!.layer.masksToBounds = true
        roomName.attributedPlaceholder = NSAttributedString(string: "Meeting ID",
                                     attributes: [NSAttributedString.Key.foregroundColor: UIColor.white])

        
        let defaultOptions = JitsiMeetConferenceOptions.fromBuilder { (builder) in
            // for JaaS replace url with https://8x8.vc
            builder.serverURL = URL(string: "https://ring.co.bw")
